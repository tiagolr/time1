...
if %1 == 1 (echo Making TIME1 Windows DEMO VERSION distribution ...) else (echo Making TIME1 Windows FULL VERSION distribution ...)
...
REM - msbuild TIME1-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild TIME1.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild TIME1.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\TIME1.aaxplugin\Contents\Win32\TIME1.aaxplugin --out .\build-win\aax\bin\TIME1.aaxplugin\Contents\Win32\TIME1.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\TIME1.aaxplugin\Contents\x64\TIME1.aaxplugin --out .\build-win\aax\bin\TIME1.aaxplugin\Contents\x64\TIME1.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\TIME1.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\TIME1.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "TIME1 Installer" ".\installer\TIME1 Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\TIME1 Installer.exe" ".\installer\TIME1 Demo Installer.exe"
REM -del ".\installer\TIME1 Installer.exe"
REM -)
