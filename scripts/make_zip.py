...
def  main():
  ...
  installer = "\installer\TIME1 Installer.exe"
   
  if demo:
    installer = "\installer\TIME1 Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\TIME1 manual.pdf" 
  ]
  ...
  ZIPNAME = "TIME1-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "TIME1-v" + FULLVERSIONSTR + "-win-demo.zip"
